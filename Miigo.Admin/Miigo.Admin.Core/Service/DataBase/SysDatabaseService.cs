...
public class SysDatabaseService : IDynamicApiController, ITransient
{
    ...
    {
        var config = App.GetOptions<DbConnectionOptions>().ConnectionConfigs.FirstOrDefault(u => u.ConfigId == input.ConfigId);
        input.Position = string.IsNullOrWhiteSpace(input.Position) ? "Miigo.Admin.Application" : input.Position;
    ...
    {
        var config = App.GetOptions<DbConnectionOptions>().ConnectionConfigs.FirstOrDefault(u => u.ConfigId == input.ConfigId);
        input.Position = string.IsNullOrWhiteSpace(input.Position) ? "Miigo.Admin.Core" : input.Position;
